module Api
  module V2
    class HostConfigGroupsController < V2::BaseController
      ...
      before_action :find_config_group, :only => [:create]
 
      api :GET, "/hosts/:host_id/config_group_ids/", N_("List all Config group IDs for host")
 
      def index
        render :json => { root_node_name => HostConfigGroup.authorized(:edit_classes).where(:host_id => @host.id).pluck('config_group_id') }
      end
      ...
      param :config_group_id, String, :required => true, :desc => N_("ID of Config Group")
 
      def create
        @host_config_group = HostConfigGroup.create!(:host_id => @host.id, :config_group_id => params[:config_group_id].to_i, :host_type => "Host::Base")
      ...
      param :host_id, String, :required => true, :desc => N_("ID of host")
      param :id, String, :required => true, :desc => N_("ID of Config Group")
 
      def destroy
        @host_config_group = HostConfigGroup.authorized(:edit_classes).where(:host_id => @host.id, :config_group_id => params[:id])
      ...
      def find_config_group
        @config_group = resource_finder(ConfigGroup.authorized(:view_config_groups),params[:config_group_id])
      end
    end
  end
end
